...
resource "aws_iam_policy" "sqs_permissions" {
  name        = "${var.prefix}-${var.environment}-sqs-permissions"
  description = "Allow Lambda to send and delete messages from SQS"
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      {
...
module "lambda" {
  ...
  description = "Publisher Lambda"
  variables = {
    QUEUE_URL = var.sqs.url
  }
  attach_policies = {
    "sqs" = aws_iam_policy.sqs_permissions.arn
  }
}
...
resource "aws_route53_record" "api_gateway" {
  zone_id = var.route53.zone_id
  name    = "publisher.${var.route53.zone}"
  type    = "A"
